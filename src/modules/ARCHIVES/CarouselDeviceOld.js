...
export default class CarouselDevice extends SpecialText {
  ...
  render(){
    //Load 'arrPics Array' with product img nodes each referenced to placeSpecial which loads specialParams array then applies params to DOM element
      ...
      for(let i=0;i<=5;i++){
          ...
          picArr.push(
            <div style = {pic} key = {i}ref = {(ref)=>this.placeSpecial(ref, i, 5)}>

              <Info text = {this.specText['i'+i]} ref = {(ref) => {this[i] = ref}}/>
                {/*<div style = {test}>text text etxtext text etxtext text etxtext text etxtext text etxtext text etx </div>*/}
...
toggleInfo = (node)=>{
        if (node == null) return
    console.log('clientRect ', node.getBoundingClientRect().bottom)
    if(node.getBoundingClientRect().bottom > 200) this.addInfo()
    }

    render(){

      //Define text content from  props.text
        let heading = this.state.info ? this.props.text.heading : ''
        let text = this.state.info ? this.props.text.text : ''
    ...
    return(
        <div title='info' ref = {(ref)=>this.toggleInfo(ref)} style = {infoBoxStyle}>
