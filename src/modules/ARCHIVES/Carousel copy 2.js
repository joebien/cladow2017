...
export default class Carousel extends Component {
  ...
  componentDidMount(){
    this.handleOver = function(){console.log('hanO fired');
    this.info.addInfo()
    }
  }
  ...
  aniSpecial(){
    ...
    console.log('specParams ',this.specialParams);
  ...
  render(){console.log('myData   ', SpecialText);
    //Load arrPics with img nodes each referenced to placeSpecial which loads specialParams array then applies params to DOM element
      ...
      let arrPics = []
      let info = this.state.infoBox

      for(let i=0;i<=5;i++){
        arrPics.push(
          <div  onMouseOver = {()=>this[i].addInfo()} onMouseLeave = {()=>this[i].addInfo('drop')}style = {pic} key = {i}ref = {(ref)=>this.placeSpecial(ref, i, 5)}>

            <Info text = {'shoobidoobi' + i} ref = {(ref) => {this[i] = ref}}/>
            <img style = {{zIndex:-11}}  key = {i}src = {require('../public/img/specials/special-' + i  + '.jpg')}/>
  ...
  return(
        <div style = {component}>
          ...
          <div><SpecialText ref = {(ref)=>{this.text = ref}}/>text</div>
        </div>
  )
  }
...
class Info extends Component{
 ...
 constructor(props) {
  ...
  addInfo(drop){console.log('addInfo Fired');
    ...
    if (drop){
      this.setState({
        ...
        infoText:''
            ...
            this.setState({
              infoBox:this.state.info,
              infoText:this.state.text
            })
          }
  ...
  render(){
    return(
      <div style = {Object.assign({},this.state.infoBox)}> {this.state.infoText}
            </div>
    )
  }
